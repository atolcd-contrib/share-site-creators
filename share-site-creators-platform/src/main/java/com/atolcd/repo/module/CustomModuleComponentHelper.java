/*
 * #%L Alfresco Repository %% Copyright (C) 2005 - 2016 Alfresco Software Limited %% This file is part of the Alfresco software. If the
 * software was purchased under a paid Alfresco license, the terms of the paid license agreement will prevail. Otherwise, the software is
 * provided under the following open source license terms:
 * 
 * Alfresco is free software: you can redistribute it and/or modify it under the terms of the GNU Lesser General Public License as published
 ...
 * #L%
 */
package com.atolcd.repo.module;
 ...
 * @author Derek Hulley
 */
public class CustomModuleComponentHelper {
  ...
  private static Log                                logger                              = LogFactory
      .getLog(CustomModuleComponentHelper.class);
  // Fin ATOLCD //

  private static Log                                loggerService                       = LogFactory.getLog(CustomModuleServiceImpl.class);
  ...
  /** Default constructor */
  public CustomModuleComponentHelper() {
    ...
    for (Map.Entry<String, Map<String, ModuleComponent>> entry : componentsByNameByModule.entrySet()) {
      ...
      for (Map.Entry<String, ModuleComponent> entryInner : componentsByName.entrySet()) {
        ...
        if (executedComponents.contains(component)) {
          ...
          String msg = I18NUtil.getMessage(ERR_COMPONENT_IN_MISSING_MODULE, componentName, moduleId);
          logger.error(msg);
        }
      }
    }
  ...
  Collection<String> getRegistryModuleIDs() {
    ...
    RegistryKey moduleKeyAllIds = new RegistryKey(CustomModuleComponentHelper.URI_MODULES_1_0, REGISTRY_PATH_MODULES, null);
  ...
  ModuleVersionNumber getVersion(String moduleId) {
    RegistryKey moduleKeyCurrentVersion = new RegistryKey(CustomModuleComponentHelper.URI_MODULES_1_0, REGISTRY_PATH_MODULES, moduleId,
  ...
  private void renameModule(ModuleDetails module) {
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug("-- Module Aliases : " + moduleAliases.toString());
      logger.debug("-- Module(s) in SystemRegistry  : " + registeredModuleIds.toString());
    }
    // FIN ATOLCD
    ...
    for (String moduleAlias : moduleAliases) {
      if (logger.isDebugEnabled()) {
        logger.debug("Current alias : " + moduleAlias);
      }
      // Debut ATOLCD //
      ...
      if (registeredModuleIds.contains(moduleAlias)) {
        ...
        RegistryKey moduleKeyNew = new RegistryKey(CustomModuleComponentHelper.URI_MODULES_1_0, REGISTRY_PATH_MODULES, moduleId, null);
        RegistryKey moduleKeyOld = new RegistryKey(CustomModuleComponentHelper.URI_MODULES_1_0, REGISTRY_PATH_MODULES, moduleAlias, null);
        // Copy it all
        ...
        logger.debug("No previously installed module has been found in the registry");
      }
    }
  }
  ...
  private void startModule(ModuleDetails module, Set<String> startedModules, Set<ModuleComponent> executedComponents) {
    ...
    if ((minRepoVersionNumber != null && repoVersionNumber.compareTo(minRepoVersionNumber) < 0)
      ...
      throw AlfrescoRuntimeException.create(ERR_UNSUPPORTED_REPO_VERSION, moduleId, moduleNewVersion, repoVersionNumber,
          minRepoVersionNumber, maxRepoVersionNumber);
    }

    // Get the module details from the registry
    RegistryKey moduleKeyInstalledVersion = new RegistryKey(CustomModuleComponentHelper.URI_MODULES_1_0, REGISTRY_PATH_MODULES, moduleId,
   ...
   */
  private void executeComponent(String moduleId, ModuleVersionNumber currentVersion, ModuleComponent component,
    ...
    String name = component.getName();
    RegistryKey executionDateKey = new RegistryKey(CustomModuleComponentHelper.URI_MODULES_1_0, REGISTRY_PATH_MODULES, moduleId,
        REGISTRY_PATH_COMPONENTS, name, REGISTRY_PROPERTY_EXECUTION_DATE);
